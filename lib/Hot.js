...
function startWatchDelay() {
	...
	this.timeout = setTimeout(updateModules.bind(this, function() {
	...
	}.bind(this)), this.options.watchDelay);
}
...
function updateModules(callback) {
	...
	if(result != "accept") {
		return callback(new Error("Cannot do hot update: " + result), true);
	}
...
Hot.prototype.check = function(callback) {
	...
	function oneDone() {
		if(--count == 0) {
			...
			updateModules.call(reqObj, function(err, updatedModules) {
				if(err) return callback(err);
				reqObj.status = "idle";
				return callback(null, updatedModules);
			});
		}
	}
}
