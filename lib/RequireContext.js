...
RequireContext.prototype.theRequire = function theRequire(modules, callback, errorCallback) {
	...
	if(Array.isArray(modules)) {
		...
		var clonedOptions = clone(this.root.options.resolve);
		if (parent && parent.paths) {
			clonedOptions.paths = clonedOptions.paths || [];
			clonedOptions.paths = clonedOptions.paths.concat(parent.paths);
		}

		// resolve request
		var request = resolve.sync(context, modules, clonedOptions);
