 ...
 */
function execLoaders(context, request, loaders, filenames, contents, cacheEntry, options, sync, callback) {
		...
		try {
			loaders.forEach(function(name) {
				...
				if(loader.separable) {
					...
					options.events && options.events.emit("static-dependency", loaderFilename);
				}
			...
			try {
				...
				var loaderContext = {
					...
					dependency: function(filename) {
						options.events && options.events.emit("dependency", filename);
						if(cacheEntry)
...
module.exports = function(context, request, loaders, filenames, contents, cacheEntry, options, callback) {
	return execLoaders(context, request, loaders, filenames, contents, cacheEntry, options, false, callback);
}
module.exports.sync = function(context, request, loaders, filenames, contents, cacheEntry, options) {
	var callback = createSyncCallback();
	execLoaders(context, request, loaders, filenames, contents, cacheEntry, options, true, callback);
	return callback.get();
}